...
puts "populating interco_communes table form opendatasoft API"
...
siren.each do |p|
   ...
   i = p["insee"] unless p["insee"].nil?
   sm = p["siren_membres"] unless p["siren_membres"].nil?
   sp = p["siren_principal"] unless p["siren_principal"].nil?
   geo = p["geometry"]["geometry"]["coordinates"] unless p["geometry"].nil?
  ...
  interco = IntercoCommune.create!(interco_commune)

puts "#{interco.id} created"
