...
before_action :set_user_program, only: [:edit, :update, :destroy]
  ...
  def create
    ...
    if user_program_params[:program].empty?
      ...
      user_program = UserProgram.new(program: user_program_params[:program], category_id: @answer.question.questionnaire.category.id, question_id: @answer.question.id )
